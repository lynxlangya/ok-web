...
const SwiperCard = () => {
  // const tmp = useRef<CarouselRef>(null);
    ...
    <div className={SwiperStyle.card__content}>
      <Carousel ref={bgRef} autoplay dots={false}>
        {list.map(item => {
          return (
            <div key={item.id}>
              <div className={SwiperStyle.card__box} style={{ backgroundImage: 'url(' + item.image + ')' }}>
                <h1 className={SwiperStyle.card__title}>{item.title}</h1>
                <h2 className={SwiperStyle.card__subtitle}>{item?.subTitle ?? ''}</h2>
              </div>
            </div>
          );
        })}
      ...
      <div className={SwiperStyle.card__step}>
        <div className={SwiperStyle.card__icon}>
          <Image priority src="/left.svg" onClick={() => {
            bgRef.current && bgRef.current.prev()
            textRef.current && textRef.current.prev()
          }} height={20} width={20} alt="icon" />
          <Image priority src="/right.svg" onClick={() => {
            bgRef.current && bgRef.current.next()
            textRef.current && textRef.current.next()
          }} height={20} width={20} alt="icon" />
        </div>
        <Carousel ref={textRef} autoplay dots={false} vertical dotPosition="left" className={SwiperStyle.item__box}>
          <div className={SwiperStyle.step_item}>引领一流的队伍，创造一流的业绩，展现一流的风彩</div>
          <div className={SwiperStyle.step_item}>用一流的人才，创一流的品质，供一流的服务，收一流的效益</div>
          <div className={SwiperStyle.step_item}>勤思巧干苦攀，风逆我自昂扬;创新创收创业，我誓更创辉煌</div>
          <div className={SwiperStyle.step_item}>诚信高效服务用户团结进取争创效益</div>
        </Carousel>
      </div>
    </div>
  )
